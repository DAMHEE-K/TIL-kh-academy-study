...
<%
	String memberId = request.getParameter("memberId"); // memberId 가져오기
...
<body>
	...
	<% if(available) { %>
		<p>
			<span id="available"><%= memberId %></span>는 사용 가능합니다.
		</p>
		<button onclick="popupClose();">확인</button>
	<% } else { %>
		<p>
			<span id="duplicated"><%= memberId %></span>는 사용할 수 없습니다.
		</p>
		<form 
	...
	<script>
	const popupClose = () => {
		// 부모창 window를 가리키는 변수 opener
		opener.document.memberEnrollFrm.memberId.value = "<%= memberId %>";
		// 부모창 문서의 memberEnrollFrm 이라는 태그 객체가 가진 memberId 라는 name 태그 객체의 value값을 memberId로 세팅
