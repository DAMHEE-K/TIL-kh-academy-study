...
<%
	...
	if(msg != null) session.removeAttribute("msg"); // 메세지를 1회용으로 만들기 위한 코드, session에 있는 msg를 지운것임
	// System.out.println("msg = " + msg);
	Member loginMember = (Member)session.getAttribute("loginMember");
	// System.out.println("loginMember = " + loginMember);
	
	Cookie[] cookies = request.getCookies(); // 요청에서 쿠키꺼냄
	String saveId = null;
	if(cookies != null) {
		...
		<header>
			...
			<% if(loginMember == null) { %>
                ...
                <form id="loginFrm" name="loginFrm" action="<%= request.getContextPath() %>/member/login" method="post">
                    <table>
                        ...
                        <tr>
                            <td><input type="password" name="password" tabindex="2" id="password" placeholder="비밀번호"></td>
                            <td></td>
                        </tr>
                        <tr>
                            <td colspan="2">
            ...
            <nav>
                <ul class="main-nav">
					...
					<% if(loginMember != null)  { %>
                    <li class="photo"><a href="<%= request.getContextPath() %>/chat/chat">채팅</a></li>
                    <% } %>
                    <%-- 관리자로 로그인 한 경우만 노출하기 --%>
