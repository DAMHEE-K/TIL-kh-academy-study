<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%-- jstl을 사용하기 위한 선언문. 매 jsp마다 필요함. --%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>JSTL - core</title>
<style>
	table {
	border: 1px solid black;
	padding:2px;
	}
	tr, td, th {
	border: 1px solid black;
	padding:5px;
	}
</style>
</head>
<body>
	<h1>JSTL - core</h1>
	<h2>변수</h2>
	<%--
		pageContext.setAttribute("no1", 10);
	--%>
	<c:set var="no1" value="100" scope="page" />
	<c:set var="no2" value="20" scope="page" />
	<ul>
		<li>no1 : <c:out value="${pageScope.no1}" /></li>
		<li>no1 : ${no1}</li>
	</ul>
	
	<h2>분기처리</h2>
	<h3>c:if</h3>
	value가 문자열로 처리되어서 문자열 연산이 되기 때문에, 숫자의 경우는 형변환 필요함<br />
	<%-- <c:if test="${no1 gt no2}">
		문자 : ${no1}이 ${no2}보다 큽니다 <br />
	</c:if> --%>
	
	<c:if test="${Integer.parseInt(no1) gt Integer.parseInt(no2)}">
		숫자로 변환 : ${no1}이 ${no2}보다 큽니다
	</c:if>
		<c:if test="${no1 lt no2}">
		${no1}이 ${no2}보다 작습니다
	</c:if>
		<c:if test="${no1 eq no2}">
		${no1}와 ${no2}는 같습니다.
	</c:if>
	
	<h3>c:choose</h3>
	<c:set var="n" value="<%= (int)(Math.random() * 5 +1) %>"></c:set>
	${n}
	<c:choose>
		<c:when test="${n eq 1}">오토바이를 뽑았습니다.</c:when>
		<c:when test="${n eq 2}">권총 라이터를 뽑았습니다.</c:when>
		<c:when test="${n eq 3}">오리 인형을 뽑았습니다.</c:when>
		<c:otherwise>꽝!! 다음 기회에.....</c:otherwise>
	</c:choose>
	
	<h3>실습문제</h3>
	<c:choose>
		<c:when test="${param.lang eq 'en'}"><p>Hello</p></c:when>
		<c:otherwise><p>안녕하세요</p></c:otherwise>
	</c:choose>
	
	<c:if test="${param.lang eq 'kr' || empty param.lang}">
		<p>안녕하세요</p>
	</c:if>
		<c:if test="${param.lang eq 'en'}">
		<p>Hello</p>
	</c:if>
	
	<h2>반복처리</h2>
	i는 무조건 작은 수에서 큰 수로 처리되어야 하며 step에는 음수가 들어갈 수 없음
	<c:forEach var="i" begin="1" end="6" step="1">
		<%-- <h${7-i}>HelloWorld${i}</h${7-i}> --%>
		<h${i}>HelloWorld${i}</h${i}>
	</c:forEach>
	
	<h3>names</h3>
	<ul>
	<%--
		items 반복 접근할 객체, el로 표기해야함 (순회할 배열)
		var 요소하나를 담을 변수명
		사용 ${var}
	 --%>
		<c:forEach items="${names}" var="name">
			<li>${name}</li>
		</c:forEach>

 
	</ul>
	<ul>
		<c:forEach items="${items}" var="item">
			<li>${item.no} ${item.name} ${item.price}</li>
		</c:forEach>
	</ul>
	<table>
		<thead>
			<tr>
				<th>No</th>
				<th>이름</th>
				<th>가격</th>
			</tr>
		</thead>
		<tbody>
			<c:forEach items="${items}" var="item">
			<tr>
				<td>${item.no}</td>
				<td>${item.name}</td>
				<td>${item.price}</td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
	<h3>map</h3>
	<ul>
		<c:forEach items="${map}" var="entry">
			<li>${entry.key} ${entry.value}</li>
		</c:forEach>
	</ul>
	<table>
		<tbody>
			<tr>
				<th>이름</th>
				<td><input type="text" value=${map.name} /></td>
			</tr>
			<tr>
				<th>나이</th>
				<td><input type="text" value=${map.age} /></td>
			</tr>
			<tr>
				<th>결혼여부</th>
				<td><input type="checkbox" ${map.married ? 'checked' : ''} />
				</td>
			</tr>
		</tbody>
	</table>
	<h2>url</h2>
	<img alt="" src="${pageContext.request.contextPath}/images/file.png">
	<img alt="" src='<c:url value="/images/file.png" />'/>
</body>
</html>