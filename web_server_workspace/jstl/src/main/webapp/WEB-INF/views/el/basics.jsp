...
<%--
	...
	1. 각 scope별 context객체의 속성(map)
		...
		- sessioScope
		- applicationScope
		- scope를 생략하면, page - request - session - application 순으로 속성 조회
 ...
 --%>
 <%
 	// jsp에서만 사용 가능한 context 객체 (쓸 일이 거의 없음)
...
<body>
	...
	<ul>
		...
		<li>${requestScope.abc}</li> <%-- null 출력안함 --%>
		<li>${requestScope.abc.def}</li>
	</ul>
	<h3>session</h3>
	<ul>
		<li>${book}</li>
	...
	<ul>
		<li>header : ${header}</li>
		<li>user-agent : ${header['user-agent']}</li> <%-- 속성명에 특수문자 포함시 [] bracket notation 사용가능 --%>
	...
	<ul>
		<li>cookie : ${cookie}</li>
		<li>JSESSIONID : ${cookie.JSESSIONID.value}</li>
	</ul>
	
	<h2>pageContext</h2>
	<%-- 유일하게 Map이 아닌 실제 pageContext 객체를 가리킨다. --%>	
	<ul>
