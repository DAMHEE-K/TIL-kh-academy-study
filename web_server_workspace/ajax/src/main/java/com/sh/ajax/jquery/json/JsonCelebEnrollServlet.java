...
public class JsonCelebEnrollServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String profile = multiReq.getFilesystemName("profile"); // 실제 저장된 파일명

		Celeb celeb = new Celeb(0, name, profile, celebType);
		
		System.out.println("name = " + name);
		System.out.println("celebType = " + celebType);
		
		// 2. 업무 로직
		int result = celebService.insertCeleb(celeb);
		
		// 3. 응답처리 - 비동기식 POST 요청은 redirect 없이 결과값을 json으로 전송
		...
		map.put("celeb", celeb);
		new Gson().toJson(map,response.getWriter()); // 응답 메세지에 src를 json문자열로 변환해서 출력
	}
