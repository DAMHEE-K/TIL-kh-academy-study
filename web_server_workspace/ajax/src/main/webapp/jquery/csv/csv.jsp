...
<body>
	...
	<br />
	<div class="ui-widget">
		<label for="classmate">우리반 친구들 : </label> <input id="classmate">
	</div>
	<script>
    $( "#classmate" ).autocomplete({
    
	/**
	* 사용자 입력값을 받아, 서버에 ajax를 요청하고, 결과를 jquery-ui쪽으로 값을 반환
	*/
    	  ...
    	  $.ajax({
    		 ...
    		 success(classmates) {
    			 ...
    			 const arr = temp.map((name) => ({
    				 ...
    				 // 원래는 다르지만 지금의 경우는 같으니까 동일한 name 대입하여 처리
    			 }));
    			 console.log(arr);
