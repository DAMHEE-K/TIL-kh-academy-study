...
create table board (
    ...
    constraints fk_board_member_id foreign key(member_id) references member(member_id) on delete set null
);
...
insert into spring.board (id,title,member_id,content,created_at) values (seq_board_id.nextval,'관리자가 공지합니다. 졸지마세요~','admin','관리자란 조직의 안정성과 계속성을 유지하고 환경에 적응하면서 쇄신적 발전을 이룩할 수 있는 여건을 조성하게 하는 역할을 가지고 있다. 그러므로 관리자는 조직 내부의 여건과 조직 환경을 고려한 각종 관리기법을 적용하여 조직을 경영하는데 최선을 다하여야 한다.',to_date('18/02/13','rr/mm/dd'));
...
insert into spring.board (id,title,member_id,content,created_at) values (seq_board_id.nextval,'인류와 수영','qwerty','세계4대 문명이 강 유역에서 그 찬란한 문화를 꽃피웠듯 인류는 물과 밀접한 관계를 맺어왔지요. 수영은 물과 인간과의 관계속에서 자연스럽게 시작되었습니다. 기원전 2500년경 고대 이집트, 그 이후의 앗시리아, 스리스 로마 문명등에서 수영의 기록을 찾아볼 수 있구요, 페르시아에서는 군사 훈련과정의 하나로 수영이 포함되었습니다. 이후 근대적 의미의 수영경기는 1837년 영국에서 처음 시작되었습니다. 우리나라에서는 1912년에 제 1회 조선 수영대회가 개최되었고, 1946년 조선 수상경기 연맹이 창설되면서 부터 수영이 본격적으로 발전하기 시작한것입니다.',to_date('18/02/15','rr/mm/dd'));

select * from attachment;
insert into attachment values(seq_attachment_id.nextval, 1, '오리지날.txt', '20202020.txt', default);
commit;
...
select * from board;
-- 데이터 추가 (조호된 것을 보드 테이블에 삽입)
insert all
    into board
    values (seq_board_id.nextval, title, member_id, content, default)
select * from board;
--delete from board where id between 2581 and 2740;
commit;

select * from attachment;

select * from board where id between 2 and 20;
